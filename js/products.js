...
async function getProductsByGender(genders) {
  try {
    ...
    if (genders.length > 0) {
      products = products.filter((product) => genders.includes(product.gender));
    }
...
function handlePageChange() {
  ...
  if (pathname === "/all") {
    getProductsByGender([]); // pass an empty array to show all products
  }
}
